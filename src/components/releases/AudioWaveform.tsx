...
export function AudioWaveform({ release, className }: AudioWaveformProps) {
  const { currentTrack, isPlaying, play, pause, currentTime: globalCurrentTime, duration: globalDuration, seekTo, setDuration } = usePlayerStore();

  // Use consistent audio URL - fallback if release.audioUrl is undefined
  const audioUrl = release.audioUrl || `https://pub-d285a77a938b46bf93539cdd85ba963b.r2.dev/Nul%20Tiel%20Records%20-%20Jeopardy.mp3?id=${release.id}`;
  ...
  const waveformData = useMemo(() => getWaveformDataForRelease(release.id, 40), [release.id]);

  // Track duration derived from release ID (in a real app, this would come from the audio file)
  const trackDuration = useMemo(() => 180 + (release.id.length % 120), [release.id]);
  ...
  const handleMouseUp = useCallback((e: MouseEvent) => {
    ...
    setIsDragging(false);
    setPreviewPercent(null);
  }, [isDragging, isCurrentTrack, globalDuration, trackDuration, getPercentFromEvent, seekTo]);
  ...
  const progressPercentage = isCurrentTrack && globalDuration > 0
    ? (globalCurrentTime / globalDuration) * 100
    : 0;

  // Preview percentage for display (while hovering or dragging)
  const showPreview = isCurrentTrack && (isHovering || isDragging) && previewPercent !== null;
  const previewTime = previewPercent !== null ? previewPercent * (globalDuration > 0 ? globalDuration : trackDuration) : 0;
  ...
  return (
    <div className={cn("flex items-center justify-between gap-2 p-2", className)}>
      <div className="flex flex-1 overflow-hidden">
        <div className="relative w-full">
          <svg
            ...
            onClick={handleClick}
            onMouseMove={(e) => isHovering && !isDragging && handleMouseMove(e)}
          ...
          >
            {waveformData.map((amplitude, index) => {
              ...
              const height = isCurrentTrack && isPlaying ? normalizedHeight : normalizedHeight * 0.3;
              const x = index * 6;
              ...
              const isInPreview = showPreview && previewPercent !== null && barPosition <= previewPercent * 100;
              ...
              return (
                <motion.rect
                  ...
                  className={cn(
                    ...
                    showPreview && isInPreview && !isPlayed && "text-primary/50"
                  )}
                  animate={{
                    scaleY: (isCurrentTrack && isPlaying && !isDragging) ? [1, 1.3, 1] : 1,
                    originY: "center"
                  }}
                  transition={{
                    duration: 0.5,
                    repeat: (isCurrentTrack && isPlaying && !isDragging) ? Infinity : 0,
                    delay: index * 0.02,
                    ease: "easeInOut"
                  }}
                />
          ...
          <div className="flex justify-between text-xs text-muted-foreground mt-1 px-1">
            <span>
              {showPreview ? (
                <span className="text-primary font-medium">{formatTime(previewTime)}</span>
              ) : (
      ...
      <button
        ...
        className="flex size-8 items-center justify-center rounded-full bg-accent text-accent-foreground hover:bg-accent/90 focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2"
